...
namespace DLSU.WizardCode.UI.Views
{
    public class ListView : MonoBehaviour
    {
        ...
        public void RefreshList()
        {
            ...
            if (dataModel != null)
            {
                foreach (object data in dataModel)
                {
                    GameObject itemView = Instantiate(itemViewPrefab, transform);
                    Debug.Assert(itemView != null, name + ": Item View not instantiated");
                    if (itemView != null)
                    {
                        ListItemView listItemView = itemView.GetComponent<ListItemView>();
                        if (listItemView != null)
                        {
                            listItemView.RaiseOnItemViewWithDataCreated(data);
                        }
                        onItemViewWithDataCreated?.Invoke(itemView, data);
